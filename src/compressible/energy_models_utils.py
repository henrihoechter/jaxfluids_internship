"""Helpers for loading energy-model data and building models."""
...
def _pad_levels(values: Sequence[float], max_levels: int) -> jnp.ndarray:
    """Pad level data to a common length."""
...
def _load_gnoffo_energy_data(
    ...
    """Load Gnoffo polynomial data for the requested species."""
...
def _load_bird_energy_data(
    ...
    """Load Bird vibrational and electronic level data."""
    raw_data = json.loads(Path(data_path).read_text(encoding="utf-8"))
...
def build_energy_model_from_config(
    ...
    *,
    ...
    if model == "gnoffo":
        ...
        from .energy_models import build_gnoffo_energy_model
    ...
    if model == "bird":
        ...
        from .energy_models import build_bird_energy_model
