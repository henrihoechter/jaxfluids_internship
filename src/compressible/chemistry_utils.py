"""Helpers for loading chemistry data from JSON files."""
...
from .chemistry_types import SpeciesTable, ReactionTable
from . import (
    constants,
    energy_models_types,
    energy_models_utils,
...
def _select_species_entries(
    ...
    """Select species entries in the requested order."""
...
def _build_vibrational_relaxation_tables(
    ...
    """Build pairwise relaxation tables for vibrating species."""
...
def check_reaction_coverage(
    ...
    Returns:
        Tuple of (included_reactions, excluded_reactions): lists of dicts with keys
        "index", "equation", and (for excluded) "missing_species".
    """
    json_path = Path(json_path)
    with json_path.open("r", encoding="utf-8") as f:
        data = json.load(f)
...
def load_reactions_from_json(
    ...
    Each reaction entry must include reactants/products dicts, Arrhenius parameters
    (C_f, n_f, E_f_over_k), flags (is_dissociation, is_electron_impact), and
    equilibrium_coeffs_casseau rows [n_ref_cm3 or n_ref_m3, A0..A4]. C_f is
    converted from per-molecule to per-mol on load; n_ref_cm3 is scaled to m^3.
...
def slice_reactions(
    ...
    Args:
        ...
        indices: Zero-based reaction indices to include.

    Returns:
        New ReactionTable with arrays sliced along the reaction dimension.
