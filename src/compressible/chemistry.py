...
from .chemistry_types import ChemistryModel, ChemistryModelConfig
...
def arrhenius_rate(
    ...
    Args:
        T_c: Controlling temperature [K].
        C_f: Pre-exponential factor [m^3/mol/s or m^6/mol^2/s].
...
def reaction_rate_products(
    ...
    Returns:
        log_prod: log(product n_s^nu_s) for each reaction.
    """
...
def equilibrium_constant_casseau(
    ...
    Args:
        T_c: Controlling temperature [K], shape (n_cells,) or (n_reactions, n_cells).
        n_mix: Mixture number density [1/m^3].
        coeffs: Casseau coefficient tables with columns [n_ref_m3, A0..A4].
...
def _dissociation_species_indices(
    ...
    Returns:
        Tuple of (indices, counts): index of the inferred dissociating species
        (argmax over candidates) and number of candidates per reaction.
    """
...
def _compute_species_production_rates(
    ...
    Args:
        ...
        net_stoich: Net stoichiometry (products - reactants).
...
def _compute_vibrational_source_cvdv_qp(
    ...
    Args:
        ...
        T_tr: Translational temperature [K].
        T_v: Vibrational temperature [K].
        species_table: Species data.

    Returns:
        Vibrational reactive source [W/m^3].
    """
    M_s = species_table.molar_masses
    D_s = species_table.dissociation_energy
    theta_v = species_table.theta_vib
...
def compute_all_chemical_sources(
    ...
    Args:
        rho_s: Partial densities [kg/m^3].
    ...
    Returns:
        Tuple of (omega_dot, Q_vib_chem): species mass production rates
        [kg/m^3/s] and vibrational reactive source [W/m^3].
    """
    ...
    # Convert K_eq to mol-based concentrations using (10^3)^{Deltanu}.
